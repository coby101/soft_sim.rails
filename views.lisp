...
(defun html.erb (aspect layout)
  (let ((method (html.erb-method (keywordify layout)))
        (before-elements (get-additional-page-elements aspect (keywordify layout) :before))
        (after-elements (get-additional-page-elements aspect (keywordify layout) :after)))
    (let ((file (layout-file-path aspect layout)))
      (with-open-file (html.erb file :direction :output :if-exists :supersede)
        (format html.erb (unparse-erb nil (format-file-notice nil (strcat layout ".html.erb"))))
